...
new_glacius = """class Glacius(BossBase):
    ...
    TUNING = {
        "global_cd":  {1:1500, 2:1100, 3:800},
        "windup":     {"punch": 500, "kick_wave": 600, "laser_beam": 800, "massive_smash": 1000},
        "cooldown":   {"punch": 800, "kick_wave": 1000, "laser_beam": 1200, "massive_smash": 1500},
    }
    ...
    def _load_sprites(self):
        ...
        def load_seq(folder_name, prefix):
            frames = []
            dir_path = os.path.join(base_path, folder_name, folder_name)
            if not os.path.exists(dir_path):
                # Fallback empty frame
                s = pygame.Surface((288, 192), pygame.SRCALPHA)
                s.fill((255, 0, 255))
                return [s]
            ...
            for i in range(1, len(files) + 1):
                ...
                if os.path.exists(path):
                    try:
                        ...
                        # Scale 192x128 -> 288x192 (x1.5)
                        surf = pygame.transform.scale(surf, (288, 192))
                        frames.append(surf)
                    except Exception:
                        pass
            if not frames:
    ...
    def _update_visual(self, dt_ms):
        ...
        # 14 frames for atk. At 30 fps, that's ~460ms.
        fps = 20
        ...
        if self.attack_name == "laser_beam" and self.attack_state in (WINDUP, EXECUTING):
            pygame.draw.circle(base, (0, 200, 255, 100), (144, 96), 140, 15)
            pygame.draw.circle(base, (255, 255, 255, 150), (144, 96), 130, 5)
