...
new_glacius = """class Glacius(BossBase):
    ...
    def _trigger_teleport(self, pr):
        ...
        target_x = pr.centerx + 400 if pr.centerx < self.rect.centerx else pr.centerx - 400
        ...
        if collision:
            ...
            for obs in self.obstacles:
                if test_rect.colliderect(obs.rect):
                    ...
                    break
            if collision2:
                target_x = pr.centerx # If totally boxed in, just go to player
    ...
    def _ex_punch(self, dt_ms):
        if self.exec_timer == 400:
            ...
            self._punch_hitbox = (cx, self.rect.centery)
    ...
    def _draw_extras(self, screen, ox, oy):
        ...
        if self.attack_state == EXECUTING:
            if self.attack_name == "punch" and getattr(self, '_punch_hitbox', None):
                hx, hy = self._punch_hitbox
                surf = pygame.Surface((120, 120), pygame.SRCALPHA)
                surf.fill((0, 150, 255, 120))
                pygame.draw.rect(surf, (255, 255, 255, 200), (0, 0, 120, 120), 4)
                screen.blit(surf, (hx - 60 + ox, hy - 60 + oy))
                
            elif self.attack_name == "kick_wave" and getattr(self, '_kick_hitbox', None):
...
print("Glacius boss successfully replaced v7!")
