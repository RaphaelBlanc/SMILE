...
new_glacius = """class Glacius(BossBase):
    ...
    def _trigger_teleport(self, pr):
        self.camp_timer = 0
        self.last_player_pos = (pr.centerx, pr.centery) # Reset to prevent immediate re-trigger
    ...
    def _start_attack(self, pr):
        ...
        if n == "laser_beam":
            ...
            self.exec_timer = 900 # Gives time for animation
            self._kick_hitbox = None
        elif n == "punch":
            self._dir = 1 if pr.centerx > self.rect.centerx else -1
            self.facing_right = self._dir == 1
    ...
    def _ex_punch(self, dt_ms):
        if self.exec_timer == 900: # Fires at start of exec
...
print("Glacius boss successfully replaced v9!")
