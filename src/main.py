    ...
    def update(self, dt):
        ...
        if self.end_time_triggered is not None:
            ...
            for m in list(self.monster_sprites):
                ...
                if not is_m_dead and getattr(m, "hitbox", m.rect).colliderect(self.player.hitbox) and self.player.hp_current > 0:
                    if getattr(m, 'contact_timer', 0) <= 0:
            ...
            for proj in list(self.player.capacite.projectiles):
                for m in list(self.monster_sprites):
                    ...
                    if not is_dead and proj.rect.colliderect(getattr(m, "hitbox", m.rect)):
                        proj.kill()
                        if not self.is_multi or self.network.role == "host":
                            m.take_damage(20)
                        elif self.network.role == "client":
