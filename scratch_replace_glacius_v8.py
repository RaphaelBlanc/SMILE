...
new_glacius = """class Glacius(BossBase):
    ...
    def _update_logic(self, player_rect, dt_ms):
        if self.attack_state not in (WINDUP, EXECUTING) and self.hp > 0:
            import math
            dist = math.hypot(player_rect.centerx - self.last_player_pos[0], player_rect.centery - self.last_player_pos[1])
            # Anti-camp: radius 200, 3 seconds
            if dist < 200:
        ...
        super()._update_logic(player_rect, dt_ms)

    def _trigger_teleport(self, pr):
        self.camp_timer = 0
        
        # Teleport exactly behind the player
        target_x = pr.centerx + 300 if pr.centerx < self.rect.centerx else pr.centerx - 300
        test_rect = self.rect.copy()
        test_rect.centerx = target_x
        
        # Clamping to wall if there's a collision to push player
        for obs in self.obstacles:
            if test_rect.colliderect(obs.rect):
                if target_x > pr.centerx: # Boss was supposed to be on the right
                    test_rect.right = obs.rect.left
                else: # Boss was supposed to be on the left
                    test_rect.left = obs.rect.right
        ...
        self.windup_timer = 2500 # Extremely slow
        self.cooldown_timer = 0
        self._start_attack(pr)
    ...
    def _get_attack_pool(self):
        ...
        if self.phase >= 3:
            pass # Phase 3 just scales speed
        return pool
    ...
    def _ex_punch(self, dt_ms):
        if self.exec_timer == 400:
            ...
            # Reduced range punch: speed = 0, radius = 50. It acts as a static melee swing.
            p = BossProjectile((cx, self.rect.centery), 0, 0, radius=50, color=ICE_BLUE, max_lifetime=150)
    ...
    def _draw_extras(self, screen, ox, oy):
        ...
        # Draw blue hitbox for kick_wave ONLY
...
print("Glacius boss successfully replaced v8!")
